...
public class TimeZoneBoundaryTest extends CoreTestFmwk {
    ...
    /**
     * Date.toString().substring() Boundary Test Look for a DST changeover to occur within 6 months
     * of the given Date. The initial Date.toString() should yield a string containing the startMode
     * as a SUBSTRING. The boundary will be tested to be at the expectedBoundary value.
     */
    void findDaylightBoundaryUsingDate(Date d, String startMode, long expectedBoundary) {
        ...
        else errln("FAIL: Expected boundary at " + expectedBoundary);
    }
    ...
    void findDaylightBoundaryUsingTimeZone(Date d, boolean startsInDST, long expectedBoundary) {
        findDaylightBoundaryUsingTimeZone(d, startsInDST, expectedBoundary, TimeZone.getDefault());
    }

    void findDaylightBoundaryUsingTimeZone(
            Date d, boolean startsInDST, long expectedBoundary, TimeZone tz) {
        // Given a date with a year start, find the Daylight onset
        // and end.  The given date should be 1/1/xx in some year.
        ...
        if (mindelta >= 0 && mindelta <= INTERVAL && mindelta >= 0 && mindelta <= INTERVAL)
            logln(
                    "PASS: Expected boundary at "
                            + expectedBoundary
                            + " = "
                            + fmt.format(new Date(expectedBoundary)));
        else
            errln(
                    "FAIL: Expected boundary at "
                            + expectedBoundary
                            + " = "
                            + fmt.format(new Date(expectedBoundary)));
    }
    ...
    private static String showDate(Date d) {
        ...
        return ""
                ...
                + ":"
                + showNN(cal.get(Calendar.MINUTE))
                + " \""
                + d
                + "\" = "
                + d.getTime();
    ...
    /**
     * Given a date, a TimeZone, and expected values for inDaylightTime, useDaylightTime, zone and
     * DST offset, verify that this is the case.
     */
    void verifyDST(
            String tag,
            Calendar cal,
            TimeZone time_zone,
            boolean expUseDaylightTime,
            boolean expInDaylightTime,
            int expRawOffset,
            int expOffset) {
        Date d = cal.getTime();
        ...
        else errln("FAIL: inDaylightTime = " + time_zone.inDaylightTime(d));

        if (time_zone.useDaylightTime() == expUseDaylightTime)
            logln("PASS: useDaylightTime = " + time_zone.useDaylightTime());
        else errln("FAIL: useDaylightTime = " + time_zone.useDaylightTime());
        ...
        if (time_zone.getRawOffset() == expRawOffset)
            ...
            errln(
                    "FAIL: getRawOffset() = "
                            + time_zone.getRawOffset() / (double) ONE_HOUR
                            + "; expected "
                            + expRawOffset / (double) ONE_HOUR);
        ...
        if (offset == expOffset) logln("PASS: getOffset() = " + offset / (double) ONE_HOUR);
        else {
            logln(
                    "era="
                            + cal.get(Calendar.ERA)
            ...
            errln(
                    "FAIL: getOffset() = "
                            + offset / (double) ONE_HOUR
                            + "; expected "
                            + expOffset / (double) ONE_HOUR);
        }
    }

    /**
     * Check that the given year/month/dom/hour maps to and from the given epochHours. This verifies
     * the functioning of the calendar and time zone in conjunction with one another, including the
     * calendar time->fields and fields->time and the time zone getOffset method.
     *
    ...
    */
    ...
    public void TestStepwise() {
        ...
        // disabled Oct 2003 aliu; ACT could mean anything, depending on the underlying JDK, as of
        // 2.8
        // findBoundariesStepwise(1997, ONE_DAY, safeGetTimeZone("ACT"), 2);
        findBoundariesStepwise(
                1997, ONE_DAY, safeGetTimeZone("America/Phoenix"), 0); // Added 3Jan01
