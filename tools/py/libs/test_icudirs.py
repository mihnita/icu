...
class TestIcuDirs(unittest.TestCase):
  ...
  def _create_fake_folder_struct(cls,
    ...
    for fake_file_name in files_to_check:
      ...
      with open(fake_file, 'w') as f: f.write("Fake, for testing") # Expected file
    pass
  ...
  def setUpClass(cls):
    ...
    cls.root_dir: tempfile.TemporaryDirectory[str] = (
        tempfile.TemporaryDirectory(prefix='icudir_test_')
    )
    # Create a fake CLDR folder
    cls._create_fake_folder_struct('icu',
      ['icu4c', 'icu4j', 'tools'],
      ['pom.xml', 'README.md', 'CONTRIBUTING.md'],
    )
    # Create a fake CLDR folder
    cls._create_fake_folder_struct('cldr',
      ['common', 'keyboards', 'specs', 'tools'],
      ['pom.xml', 'README.md', 'CONTRIBUTING.md']
    )
    # Create a fake CLDR-staging folder
    cls._create_fake_folder_struct('cldr-staging',
